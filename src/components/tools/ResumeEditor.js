...
import { updateResume, updateResumeEditorStatus } from "../../actions";
...
export class ResumeEditor extends Component {
  ...
  isValidJSON = data => {
    ...
    try {
      ...
      if (
        !("technicalSkills" in keys) &&
        !Array.isArray(newResume.technicalSkills)
      ) {
        throw new Error("");
      }
      ...
      if (
        !("certification" in keys) &&
        !Array.isArray(newResume.certification)
      ) {
        throw new Error("");
      }
      cleanedResume = {
